...
import net.opentsdb.client.util.LocalHost;
import net.opentsdb.client.util.NIOHelper;
...
public class AgentName extends NotificationBroadcasterSupport  implements AgentNameMBean {
  ...
  /** The JVM's PID */
	...
	private AgentName() {
		super(NOTIF_INFOS);
		forceLowerCase = getBooleanSystemThenEnvProperty(PROP_FORCE_LOWER_CASE, DEFAULT_FORCE_LOWER_CASE);
		shortHostName = getBooleanSystemThenEnvProperty(PROP_USE_SHORT_HOSTNAMES, DEFAULT_USE_SHORT_HOSTNAMES); 				
		loadExtraTags();
		getAppName();
		getHostName();
		initBufferized();
		try {
			...
			log.warn("Failed to register management interface for AgentName. Continuing without", ex);
		}
	...
	public static String remoteAppName(final MBeanServerConnection remote) {
		...
		try {
			final ObjectName runtimeMXBean = new ObjectName(ManagementFactory.RUNTIME_MXBEAN_NAME);
	...
	public static String remoteHostName(final MBeanServerConnection remote) {
		...
		final boolean forceLowerCase = getBooleanSystemThenEnvProperty(PROP_FORCE_LOWER_CASE, DEFAULT_FORCE_LOWER_CASE);
		final boolean shortHost = getBooleanSystemThenEnvProperty(PROP_USE_SHORT_HOSTNAMES, DEFAULT_USE_SHORT_HOSTNAMES); 				
	...
	public static String getSysPropAppName() {
		...
		return getSystemThenEnvProperty(appProp, null);
	}
